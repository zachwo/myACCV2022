...
Mat RGB2YCbCr(Mat img) {
	...
	int height = img.rows;
	float dummy;
	Mat img_copy(height, width, CV_8UC3, Scalar(0, 0, 0)); //定义一张与img同尺寸同格式的空白图片
	...
	for (int i = 0; i < height; i++) {
		for (int k = 0; k < width; k++) { // indexing every channels of a 2-D array form of images.
			...
			img_copy.at<Vec3b>(i, k)[2] = dummy;  //数组的第三位是Y值
			...
			img_copy.at<Vec3b>(i, k)[1] = dummy; //数组的第二位是Cb值
			...
			img_copy.at<Vec3b>(i, k)[0] = dummy;//数组的第一位是Cr值
